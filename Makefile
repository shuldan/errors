...
fmt: $(GOIMPORTS)
	...
	@$(GOIMPORTS) -local github.com/shuldan/errors -w $$(find . -name "*.go" -not -path "./vendor/*")
...
fmt-check: $(GOIMPORTS)
	...
	@unformatted_imports=$$($(GOIMPORTS) -local github.com/shuldan/errors -l . | grep -v vendor || true); \
